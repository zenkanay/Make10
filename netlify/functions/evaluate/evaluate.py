...
def handler(event, context):
    ...
    latex_str = body.get("latex")
    target_numbers = body.get("numbers")
    api_key = body.get("api_key")
    
    if latex_str is None or target_numbers is None:
        return {
            "statusCode": 400,
            "headers": cors_headers,
            "body": json.dumps({"error": "Missing latex or numbers parameter"})
        }
    ...
    # Fallback to Gemini if SymPy failed or returned non-numeric (or symbolic), and API key is available
    ...
    # Final rescue: if everything failed but we have a symbolic expression from SymPy,
    ...
    # Check if Make 10 condition is met
    is_make10 = False
