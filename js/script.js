...
const TRANSLATIONS = {
    ja: {
        ...
        get_btn: "取得",
        myscript_key_help: "手書き入力で数式を認識するためのAPIキーです。developer.myscript.com で取得できます（月2,000回まで無料）。キーはLocalStorageにのみ保存されます。",
        ...
        help_rule_1: "与えられた4つの数字を1回ずつ使い、結果が「10」になる数式を作ります。",
        help_rule_2: "四則演算（＋－×÷）に加え、ルート(√)、階乗(!)、微積分などの記号も使えます。",
    ...
    en: {
        ...
        close_share_modal_aria: "Close share",
        ...
        help_rule_1: "Use the 4 given numbers once each to create a formula that equals 10.",
        help_rule_2: "Supports basic math (+－* /), square roots (√), factorials (!), and calculus.",
...
function prepareShareImage() {
    ...
    ctx.font = "bold 16px 'Outfit', -apple-system, sans-serif";
    const logoX = 30;
    const logoY = 30;
    ...
    ctx.lineWidth = 1;
    roundRect(ctx, 30, 42, 540, 410, 16); // Lowered Y to 42, compact height to 410
    ctx.fill();
    ctx.stroke();
    ...
    ctx.fillText(TRANSLATIONS[currentLang].available_digits || "使える数字", 60, 72); // 90 -> 72

    const digitGap = 14;
    const digitSize = 76; // Medium size digits
    const startX = 127; // centered: (540 - (4*76 + 3*14))/2 + 30 = 127
    const startY = 82; // 104 -> 82
    ...
    ctx.fillStyle = textMuted;
    ctx.font = "bold 14px 'Outfit', -apple-system, sans-serif";
    ctx.fillText(TRANSLATIONS[currentLang].formula_input || "数式入力", 60, 180); // 212 -> 180

    const inputX = 60;
    const inputY = 192; // 227 -> 192
    const inputW = 480;
